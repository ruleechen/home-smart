...
#ifdef EC_APP_RAM_TEST

#include "ec_core.h"

#define EC_APP_UART0_TX_BUF_SIZE 100                  //串口0发送缓冲区大小，可以根据需要调整
#define EC_APP_UART0_RX_BUF_SIZE 100                  //串口0接收缓冲区大小，可以根据需要调整
uint8_t uart0_tx_buf[EC_APP_UART0_TX_BUF_SIZE] = {0}; //串口0发送缓冲区
uint8_t uart0_rx_buf[EC_APP_UART0_RX_BUF_SIZE] = {0}; //串口0接收缓冲区

uint8_t ram_test_buf[11000] = {0}; //测试有多少RAM可以给App使用
...
int main(void)
{
    ...
    ec_core_uart0_printf("ec_app_ram_test %d \r\n", ram_test_buf[0]); //串口0 printf打印
