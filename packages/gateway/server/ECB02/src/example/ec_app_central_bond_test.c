 ...
 *-----------------------------------------------------------------------------
 * @file        ec_app_central_bond_test.c
 * @brief       central bond demo
...
#ifdef EC_APP_CENTRAL_BOND_TEST
...
int main(void)
{
    ...
    uint8_t mac[6] = {0x20, 0x22, 0x02, 0x17, 0x12, 0x08};
    ec_core_ble_central_bond_mac(mac); //配置主机需要绑定的蓝牙从机MAC地址

    ec_core_init(); //蓝牙内核初始化

    uart0_init();                                         //串口0初始化 波特率精度受时钟频率影响
    ec_core_uart0_printf("ec_app_central_bond_test\r\n"); //串口0 printf打印
